...
namespace mae
{
	...
	void SelectingTarget::getMarkerInRange()
	{
		...
		std::vector<Marker*> markerInRangeTmp = properties_.robot->getMarkerInRange(properties_.stock->getInUse());
		if(properties_.currentMarker == NULL) {
			...
			for(Marker *marker : markerInRangeTmp) {
				if(properties_.currentMarker != marker)
	...
	bool SelectingTarget::checkBlankSpace()
	{
		...
		if(markerCount == 0) {
			...
			properties_.angleToTurn = properties_.robot->getAngleTo(markerInRange_[0]) + MAX_ANGLE_DIFF;
			return true;
		}
		...
		for(int i = 0; i < markerCount; ++i) {
			double angle = properties_.robot->getAngleTo(markerInRange_[i]);
	...
	void SelectingTarget::getMarkerTarget()
	{
		for(Marker *marker : markerInRange_) {
			if(properties_.nextMarker == NULL || properties_.nextMarker->value > marker->value)
